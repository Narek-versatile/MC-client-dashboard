...
class BotManager extends EventEmitter {
  ...
  updateConfig(patch = {}) {
    ...
    this.log(null, `Config updated — server ${this.config.host}:${this.config.port}, transit "${this.config.transitCommand}" (GUI-click ${this.config.guiClick ? 'on' : 'off'}, debug ${this.config.debug ? 'on' : 'off'}). Existing bots reconnect on next disconnect.`);
  }
  ...
  _attachHandlers(record, account, bot) {
    ...
    bot.on('spawn', () => {
      ...
      this.debug(username, `spawn — dimension=${bot.game && bot.game.dimension}, gameMode=${bot.game && bot.game.gameMode}, server brand=${(bot._client && bot._client.brand) || 'n/a'}`);
      this.emitState();
    ...
    bot.on('end', (reason) => {
      ...
      // Don't wipe a Survival location if this 'end' is the transfer itself.
      if (!near) record.location = 'Lobby';
      clearTimeout(record.authTimer);
  ...
  _attachDiagnostics(record, bot) {
    ...
    bot.on('resourcePack', () => this.debug(username, 'event: resourcePack requested by server'));
