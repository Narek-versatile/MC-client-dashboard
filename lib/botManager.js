...
const DEFAULTS = {
  host: 'localhost',
  port: 25565,
  version: '1.18.2',         // default protocol; blank/false = auto-detect
                             // (1.18.2 avoids the 1.20.2+ config-phase transfer
                             //  bug that breaks server transfers on some networks)
  ...
  outputWindowMs: 3000,      // how long to capture chat as a command's output
  tpaWaveSize: 5,            // bots per Wave TPA batch (server caps pending reqs)
  tpaWaveIntervalMs: 10000,  // gap between Wave TPA batches
};
...
class BotManager extends EventEmitter {
  ...
  // Global chat feed. Multiple bots see the same server messages, so dedupe
  // identical lines within a short window to keep the feed clean.
  _emitChat(record, text) {
    const clean = text.replace(/§[0-9a-fk-or]/gi, '').trimEnd(); // strip § color codes
    if (!clean.trim()) return;

    const now = Date.now();
    if (!this._chatSeen) this._chatSeen = new Map();
    const last = this._chatSeen.get(clean);
    if (last && now - last < 1500) return; // duplicate from another bot
    this._chatSeen.set(clean, now);
    // Prune the dedupe cache occasionally.
    if (this._chatSeen.size > 200) {
      for (const [k, t] of this._chatSeen) {
        if (now - t > 5000) this._chatSeen.delete(k);
      }
    }

    const entry = { via: record.username, text: clean, time: now };
    this.emit('chat', entry);
    this._appendFile({ username: `chat:${record.username}`, level: 'chat', message: clean, time: now });
  }

  // After a command is sent with "show output", capture the bot's incoming
  // chat for a short window and surface it as that command's output. Minecraft
  // has no command/response correlation, so this is a best-effort time window.
  _captureOutput(record, text) {
    const cap = record.captureOutput;
    if (!cap) return;
    if (Date.now() > cap.until) { record.captureOutput = null; return; }
    const clean = text.replace(/§[0-9a-fk-or]/gi, '').trimEnd();
    if (!clean.trim()) return;
    this.emit('output', { via: record.username, command: cap.command, text: clean, time: Date.now() });
  }
  ...
  getState() {
    ...
    return {
      ...
      config: {
        ...
        tpaWaveSize: this.config.tpaWaveSize,
        tpaWaveIntervalSec: Math.round(this.config.tpaWaveIntervalMs / 1000),
      },
    };
  }
  ...
  _attachHandlers(record, account, bot) {
    ...
    // --- Messages: conditional auth + global chat feed --------------------
    bot.on('message', (jsonMsg) => {
      ...
      if (!text) return;
      this._handleAuthMessage(record, password, text);
      this._emitChat(record, text);
      this._captureOutput(record, text);
    });
  ...
  // -------------------------------------------------------------------------
  sendCommand(target, message, opts = {}) {
    const raw = String(message || '').trim();
    if (!raw) return;

    // Commands only: never let anything go to global chat. Strip any leading
    // slashes and re-add exactly one, so the bot always issues a command.
    ...
    for (const record of targets) {
      try {
        if (opts.showOutput) {
          record.captureOutput = { command, until: Date.now() + this.config.outputWindowMs };
        }
        record.instance.chat(command);
        this.log(record.username, `» ${command}`, 'info');
      } catch (err) {
        this.log(record.username, `Failed to send "${command}": ${err.message}`, 'error');
      }
    }
  }
  ...
  // -------------------------------------------------------------------------
  // Wave TPA — all online bots /tpa <player>, in batches, so the number of
  // pending requests never exceeds what the server lets you hold at once.
  // -------------------------------------------------------------------------
  waveTpa(player, opts = {}) {
    const target = String(player || '').trim().replace(/^\/+/, '');
    if (!target) {
      this.log(null, 'Wave TPA needs your in-game name (the player to teleport to).', 'warn');
      return;
    }

    const waveSize = Math.max(1, Number(opts.waveSize) || this.config.tpaWaveSize);
    const intervalMs = Math.max(1000, Number(opts.intervalMs) || this.config.tpaWaveIntervalMs);

    // Snapshot the online bots now; each is re-checked at send time in case it
    // dropped during the waits.
    const online = this._resolveTargets('__all__');
    if (online.length === 0) {
      this.log(null, 'No online bots available for Wave TPA.', 'warn');
      return;
    }

    // Restart cleanly if a wave run is already in progress.
    this.cancelTpaWave(true);

    const waves = [];
    for (let i = 0; i < online.length; i += waveSize) waves.push(online.slice(i, i + waveSize));

    this.log(null, `Wave TPA to "${target}": ${online.length} bot(s) in ${waves.length} wave(s) of ${waveSize}, ${intervalMs / 1000}s apart.`, 'info');

    let idx = 0;
    const runWave = () => {
      const wave = waves[idx];
      if (!wave) {
        this.log(null, 'Wave TPA complete.', 'success');
        this._tpaWaveTimer = null;
        return;
      }
      idx++;
      const names = wave.map((r) => r.username).join(', ');
      this.log(null, `Wave ${idx}/${waves.length} → /tpa ${target}: ${names}`, 'info');
      for (const snap of wave) {
        const rec = this.bots.get(snap.username);
        if (rec && rec.status === 'online' && rec.instance) {
          try {
            rec.instance.chat(`/tpa ${target}`);
            this.log(rec.username, `» /tpa ${target}`, 'info');
          } catch (err) {
            this.log(rec.username, `Failed /tpa: ${err.message}`, 'error');
          }
        } else {
          this.log(snap.username, 'Skipped /tpa — no longer online.', 'warn');
        }
      }
      if (idx < waves.length) {
        this._tpaWaveTimer = setTimeout(runWave, intervalMs);
      } else {
        this.log(null, 'Wave TPA complete.', 'success');
        this._tpaWaveTimer = null;
      }
  ...
  cancelTpaWave(silent = false) {
    if (this._tpaWaveTimer) {
      clearTimeout(this._tpaWaveTimer);
      this._tpaWaveTimer = null;
      if (!silent) this.log(null, 'Wave TPA cancelled.', 'warn');
      return true;
    }
    if (!silent) this.log(null, 'No Wave TPA in progress.', 'info');
    return false;
  }
}
