...
class BotManager extends EventEmitter {
  ...
  _attachHandlers(record, account, bot) {
    ...
    bot.on('windowOpen', (window) => {
      ...
      try {
        // Search the container's own slots (excluding the appended player
        // inventory) for the actual iron_pickaxe, rather than trusting a
        // hardcoded position — GUI layouts vary by server/version and a
        // fixed slot index is fragile.
        const containerEnd = typeof window.inventoryStart === 'number'
        ...
        for (let i = 0; i < containerEnd; i++) {
          ...
          if (it && it.name === 'iron_pickaxe') { slot = i; break; }
        }
